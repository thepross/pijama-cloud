    ...
    <header
        ...
        <div class="flex items-center gap-3">
            ...
            <form @submit.prevent="triggerFullSearch" class="relative w-48 sm:w-64">
                ...
                <Input type="search" placeholder="Buscar en todo el sitio..." v-model="searchQuery"
                    class="pl-8 h-9 text-sm" @focus="showResults = true" @blur="handleBlur" />
                <!-- Dropdown results panel -->
                <div v-if="showResults && (isSearching || searchResults.length > 0 || (searchQuery.length >= 2 && searchResults.length === 0))"
                    ...
                    <div v-if="isSearching"
                        class="p-3 text-center text-sm text-muted-foreground flex items-center justify-center gap-2">
                    ...
                    <div v-else class="space-y-1">
                        <Link v-for="item in searchResults" :key="item.id + '-' + item.type"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-medium truncate flex items-center gap-2">
                                    ...
                                    <span
                                        class="text-[10px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded bg-primary/10 text-primary shrink-0">
                                        {{ item.type }}
                                    </span>
                                </div>
            ...
            <Dialog>
                ...
                <DialogContent class="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Adapta el diseño a tus preferencias visuales.
                        </DialogDescription>
                    </DialogHeader>
