            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
                        <form @submit.prevent="submit" class="p-6 sm:p-8 space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <select id="id_pedido" v-model="form.id_pedido"
                                    ...
                                    <option v-for="pedido in props.pedidos" :key="pedido.id" :value="pedido.id">
                                        Pedido #{{ pedido.id }} — {{ pedido.fecha_pedido }} (Saldo: Bs. {{
                            ...
                            <div v-if="selectedOrder"
                                ...
                                <div class="space-y-0.5">
                                    <span
                                        class="text-[10px] text-muted-foreground uppercase font-bold tracking-wide">Total
                                        Pedido</span>
                                    <p class="font-mono text-sm font-bold text-foreground">Bs. {{
                                        Number(selectedOrder.total).toFixed(2) }}</p>
                                </div>
                                <div class="space-y-0.5">
                                    <span
                                        class="text-[10px] text-muted-foreground uppercase font-bold tracking-wide font-medium">Abonado</span>
                                    <p class="font-mono text-sm font-bold text-emerald-600 dark:text-emerald-400">Bs. {{
                                        Number(selectedOrder.total_pagado).toFixed(2) }}</p>
                                </div>
                                <div class="space-y-0.5">
                                    <span
                                        class="text-[10px] text-muted-foreground uppercase font-bold tracking-wide font-medium">Saldo
                                        Pendiente</span>
                                    <p class="font-mono text-sm font-bold text-amber-600 dark:text-amber-400">Bs. {{
                                        Number(selectedOrder.saldo_pendiente).toFixed(2) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <span
                                            class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-mono">$</span>
                                        <Input id="monto" v-model="form.monto" type="number" step="0.01"
                                            placeholder="0.00" class="pl-7 rounded-xl" />
                                    </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <select id="numero_cuota" v-model="form.numero_cuota"
                                        class="flex h-10 w-full rounded-xl border border-border bg-card px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <Input id="total_cuotas" v-model="form.total_cuotas" type="number" min="1"
                            ...
                            <div v-if="form.tipo_pago === 'efectivo'"
                                ...
                                <div>
                                    <span class="font-bold text-foreground block">Instrucciones de Pago en
                                        Efectivo</span>
                                    Al registrar el pago en efectivo, este quedará en estado <strong
                                        class="text-amber-600 dark:text-amber-400">pendiente</strong> hasta ser validado
                                    por un administrador.
                                </div>
                            </div>


                            <div v-if="form.tipo_pago === 'qr'"
                                class="p-4 rounded-xl border border-border bg-muted/40 text-xs text-muted-foreground flex gap-2.5 animate-in slide-in-from-bottom-2 fade-in">
                            ...
                            <div v-if="form.tipo_pago === 'tarjeta'"
                                ...
                                <span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Datos de
                                    la Tarjeta</span>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        <Label for="card_holder" class="text-xs font-semibold text-foreground">Nombre
                                            del Titular</Label>
                                        <Input id="card_holder" v-model="cardHolder" placeholder="JUAN PEREZ"
                                            class="rounded-xl" />
                                    </div>
                                    <div class="space-y-2">
                                        <Label for="card_number" class="text-xs font-semibold text-foreground">Número de
                                            Tarjeta</Label>
                                        <Input id="card_number" v-model="cardNumber" placeholder="4000 1234 5678 9010"
                                            class="rounded-xl" />
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        <Label for="card_expiry" class="text-xs font-semibold text-foreground">Fecha
                                            Expiración</Label>
                                        <Input id="card_expiry" v-model="cardExpiry" placeholder="MM/AA"
                                            class="rounded-xl" />
                                    </div>
                                    <div class="space-y-2">
                                        <Label for="card_cvv" class="text-xs font-semibold text-foreground">CVV</Label>
                                        <Input id="card_cvv" v-model="cardCvv" type="password" maxlength="4"
                                            placeholder="•••" class="rounded-xl" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                <Label for="observacion" class="text-sm font-semibold text-foreground">Nota /
                                    Observación</Label>
                                <textarea id="observacion" v-model="form.observacion" rows="3"
                                    ...
                                    class="flex min-h-[60px] w-full rounded-xl border border-border bg-card px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"></textarea>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div v-if="form.tipo_pago === 'tarjeta'"
                        ...
                        <div
                            class="relative w-full h-48 rounded-xl bg-gradient-to-tr from-slate-900 via-indigo-950 to-slate-900 text-white p-6 shadow-md overflow-hidden flex flex-col justify-between select-none">

                            <div class="absolute -right-10 -top-10 w-32 h-32 rounded-full bg-primary/20 blur-xl"></div>
                            <div class="absolute -left-10 -bottom-10 w-32 h-32 rounded-full bg-indigo-500/20 blur-xl">
                            </div>

                            <div class="flex items-start justify-between">
                                <div class="space-y-1">
                                    <span class="text-[8px] uppercase tracking-widest text-slate-400 font-bold">PIJAMAS
                                        CLOUD SECURE</span>
                                    <div
                                        class="h-7 w-10 rounded bg-amber-500/80 border border-amber-400/40 relative overflow-hidden">
                            ...
                            <div class="space-y-4">
                                ...
                                <div
                                    class="flex items-center justify-between text-[9px] uppercase tracking-wider text-slate-300">
                                    <div class="space-y-0.5">
